Main = {
	...
	prev_portfolio_img: function(e){
		var self = $(this),
		img = self.next().find('img'),
		src = img.attr('src'),
		src_segments = src.split('screenshot_'),
		target = src_segments[src_segments.length - 1].split('.')[0];

		number = --target;

		if(number > 0 && self.prev().attr('data-total') != 1){
			self.next().next().show();
		}

		if(number > 0){
			img.attr('src', src_segments[0] + 'screenshot_' + number + '.png');
		}
		if(number < 2){
			self.hide();
		}
	...
	next_portfolio_img: function(e){
		var self = $(this),
		img = self.prev().find('img'),
		src = img.attr('src'),
		src_segments = src.split('screenshot_'),
		target = src_segments[src_segments.length - 1].split('.')[0];
		...
		if(number == self.prev().attr('data-total')){
			self.hide();
		}
